...
semantic.ready = function() {
    ...
    var
        ...
        $restDocumentor          = $('.rest-documentor'),
        $shownExample     = $restDocumentor.filter('.shown'),
...
//        },
        ...
        overviewMode: function() {
            ...
            if($body.hasClass('overview')) {
                ...
                $restDocumentor.each(function() {
                    $(this).children().not('.ui.header:eq(0), .example p:eq(0)').hide();
                });
                $restDocumentor.filter('.another').hide();
            }
            else {
                ...
                $restDocumentor.filter('.another').show();
            }
        },
        developerMode: function() {
            var
                $restDocumentor = $('.rest-documentor').not('.no')
                ;
        ...
        generateCode: function() {
            var
                $restDocumentor    = $(this).closest('.rest-documentor'),
                $annotation = $restDocumentor.find('.annotation'),
                $code       = $annotation.find('.code'),
                $header     = $restDocumentor.children('.ui.header:first-of-type').eq(0).add('p:first-of-type'),
                $demo       = $restDocumentor.children().not($header).not('i.code:first-child, .code, .instructive, .language.label, .annotation, br, .ignore, .ignored'),
                code        = ''
                ;
            if( $code.size() === 0) {
                $demo
        ...
        createCode: function(type) {
            var
                ...
                $code       = $annotation.find('.code'),
                $demo       = $restDocumentor.children().not($header).not('i.code:first-child, .code, .instructive, .language.label, .annotation, br, .ignore, .ignored'),
                code        = $restDocumentor.data('code') || $.proxy(handler.generateCode, this)()
                ;
